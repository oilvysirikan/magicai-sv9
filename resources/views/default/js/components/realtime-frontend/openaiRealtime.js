...
export default () => ( {
	...
	stop() {
		if ( !this.lastResponseSaved && 'saveResponseAsync' in window && this.lastUserQuestion.trim() !== '' && this.lastAiResponse.trim() !== '' ) {
			const aiChatBubble = this.lastAiBubble.closest('.lqd-chat-ai-bubble');
	...
	createChatBubble( role ) {
		...
		if ( role === 'user' ) {
			...
			if ( canvasModeActivated && canvasEditBtnTemplate ) {
				const canvasEditBtn = canvasEditBtnTemplate.content.cloneNode(true);
				bubbleContainer.closest( '.chat-content-container' )?.insertAdjacentElement('afterbegin', canvasEditBtn.firstElementChild);
			}
		}
