{{-- One Video Studio output card; rendered inside an x-for binding `entry`. --}}
<x-card
...
>
    <figure class="relative aspect-[1/0.7] overflow-hidden bg-foreground/5">
        <template x-if="entry.status === 'completed' && entry.thumb_url">
            <img
        ...
        <template x-if="entry.status === 'completed' && !entry.thumb_url && entry.video_url">
            <video
                class="size-full object-contain"
                :src="entry.video_url + (entry.video_url.includes('#') ? '' : '#t=0.1')"
                preload="metadata"
                muted
                playsinline
            ></video>
        ...
        <template x-if="entry.status === 'failed'">
            <div
                class="group/failed inline-grid size-full cursor-help place-items-center bg-background/5 px-2 text-center"
        ...
        {{-- Source badge --}}
        <span
            class="absolute start-2 top-2 z-3 rounded-full bg-black/60 px-2 py-0.5 text-3xs font-medium text-white backdrop-blur-sm"
            x-show="entry.source_label"
            x-text="entry.source_label"
        ></span>
        ...
        <div class="flex items-center gap-2">
            ...
            >
                ...
                <x-slot:dropdown>
                    ...
                    <template x-if="entry.rename_url">
                        <x-button
                            class="w-full justify-start !rounded-md text-start hover:transform-none"
                            variant="none"
                            hover-variant="primary"
