<script>
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('voiceoverPickers', () => ({
            ...
            init() {
                this._languagesEl = document.getElementById('languages');
                this._voiceEl = document.getElementById('voice');

                if (!this._languagesEl || !this._voiceEl) {
                    return;
                }

                this.refreshLanguages();
                this.refreshVoices();
                this.selectedLanguage = this._languagesEl.value;
                this.selectedVoice = this._voiceEl.value;

                // populateVoiceSelect() rebuilds #voice options via jQuery; catch that and resync.
                this._voiceObserver = new MutationObserver(() => {
                    this.refreshVoices();
                    this.selectedVoice = this._voiceEl.value;
                });
                this._voiceObserver.observe(this._voiceEl, {
                    childList: true
                });

                // Some flows call .val(x).change() on the native selects — mirror those too.
                this._languagesEl.addEventListener('change', () => {
                    this.selectedLanguage = this._languagesEl.value;
                });
            ...
            refreshLanguages() {
                this.languageOptions = Array.from(this._languagesEl.options).map((o) => ({
                    value: o.value,
                    label: (o.textContent || '').trim(),
                    language: o.getAttribute('language') || '',
                    abbr: o.getAttribute('abbr') || '',
                }));
            },
            ...
            refreshVoices() {
                this.voiceOptions = Array.from(this._voiceEl.options).map((o) => ({
                    value: o.value,
                    label: (o.textContent || '').trim(),
                    platform: o.getAttribute('platform') || '',
            ...
            get filteredVoices() {
                const query = (this.voiceQuery || '').trim().toLowerCase();
                if (!query) return this.voiceOptions;
                return this.voiceOptions.filter((o) => {
                    const haystack = [o.label, o.value, o.platform, o.name]
                        .filter(Boolean)
                        .join(' ')
                        .toLowerCase();
                    return haystack.includes(query);
            ...
            get activeLanguageLabel() {
                const match = this.languageOptions.find((o) => o.value === this.selectedLanguage);
                return match?.label || "{{ __('Select a language') }}";
            },

            get activeVoiceLabel() {
                const match = this.voiceOptions.find((o) => o.value === this.selectedVoice);
                return match?.label || "{{ __('Select a voice') }}";
            },

            selectLanguage(value) {
                this._languagesEl.value = value;
                this.selectedLanguage = value;
                // Trigger jQuery + native change listeners (populateVoiceSelect + populatePaceSelect).
                $(this._languagesEl).trigger('change');
            },

            selectVoice(value) {
                this._voiceEl.value = value;
                this.selectedVoice = value;
                $(this._voiceEl).trigger('change');
            },

            previewUrlFor(voiceId) {
                if (typeof elevenLabsVoices === 'undefined' || !Array.isArray(elevenLabsVoices)) {
                    return null;
                }
                const match = elevenLabsVoices.find((v) => v && v.voice_id === voiceId);
